...
def _fetch_requesty_models() -> List[Tuple[str, str]]:
    """Fetch available models from Requesty, newest first.

    Uses the curated managed list (no auth needed) and falls back to the
    full catalog if that request fails.
    """
    import requests
    for url in (
        ...
        try:
            ...
            console.print(f"\n[yellow]Could not fetch Requesty models from {url}: {e}[/yellow]")
    return []


def select_requesty_model() -> str:
    ...
    choice = questionary.select(
        "Select Requesty Model (latest available):",
        choices=choices,
        instruction="\n- Use arrow keys to navigate\n- Press Enter to select",
        style=questionary.Style([
            ("selected", "fg:magenta noinherit"),
