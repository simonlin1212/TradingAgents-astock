...
- [x] T-009: 本地端到端冒烟(需本人 claude setup-token 登录 Max,无法代跑;步骤见 README「用 Max 订阅额度」)——`claude setup-token` → 启用 override → 跑一次真实分析,确认 research_manager/portfolio_manager 走 Agent SDK 且订阅额度被消耗、`ANTHROPIC_API_KEY` 未共存(AC-001/AC-002)~30min
