...
可选 provider `claude_agent_sdk`（`tradingagents/llm_clients/claude_agent_sdk_client.py`，可选依赖 `[agentsdk]`），让节点经 Claude Agent SDK 走**个人 Pro/Max 订阅额度**而非按 token 计费的 API。两档覆盖：`deep_think_provider_override="claude_agent_sdk"` 只覆盖深度节点（Research/Portfolio Manager）；再加 `quick_think_provider_override="claude_agent_sdk"` 则**含 7 个工具分析师**（即全节点走订阅），Web 侧栏「订阅覆盖」三档=关闭/仅深度/所有节点。分析师是工具节点：`bind_tools` 把 LangChain 工具桥接成 Agent SDK 进程内 MCP 工具（`create_sdk_mcp_server`+`@tool`），SDK 内部跑完 ReAct 工具循环、返回 0-tool_calls 最终报告 → LangGraph 视为完成；撞额度时 fallback 的 `bind_tools` 重新汇入外部 ToolNode 循环（无需改图）。关键点：`claude-agent-sdk` 顶 httpx>=0.28.1 与 mootdx 冲突（故可选）；无 `CLAUDE_CODE_OAUTH_TOKEN` 时用本机已登录 `claude`（钥匙串）——headless/CI 才需 `claude setup-token`；**F-004 护栏**启动时拒 `ANTHROPIC_API_KEY` 共存（防悄悄回 API 计费）；**F-005** 撞额度/失败自动降级到 fallback provider；`RateLimitEvent` **只有 `status=='rejected'` 才降级**（`overage_status=='rejected'` 是 overage 禁用的常驻位、不能当降级信号，否则 100% 静默降级）。仅覆盖 Claude 模型、订阅额度共享无 SLA、发布给他人须另取 Anthropic 批准。详见 `specs/1.agent-sdk-max-provider/` 与 `specs/LESSONS.md`。
